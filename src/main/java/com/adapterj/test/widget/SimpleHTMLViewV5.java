...
public class SimpleHTMLViewV5 extends AbstractView implements Javable, Formattable {

	private static final long serialVersionUID = 6205756869666618426L;
	private static final boolean DEBUG = Debugger.DEBUG ? true : false;
    private static final String TAG = SimpleHTMLViewV5.class.getName();
	...
	private static final String SIMPLE_FORM_VAR_LINE5      = "final java.util.List %s = %s.getAllTextGroup();";
    ...
    private static final String SIMPLE_VIEW_VAR_LINE5      = "final java.util.List %s = %s.getAllTextGroup();";
    ...
    private static final String SIMPLE_MAP_TEXT_VALUE      = "<!--<![CDATA[JAVA[= %s.getText(%d).%s() ]]]>-->"; // ?????? 
	 ...
	 * 
	 */
	public SimpleHTMLViewV5() {
		try {
			_acceleratorClass = SimpleHTMLViewAcceleratorV2.class.getName();
	 ...
	 */
	public SimpleHTMLViewV5(final String acceleratorClass) {
	 ...
	 */
	public SimpleHTMLViewV5(final Document document, final String md5) {
		if (document == null) {
	 ...
	 */
	public SimpleHTMLViewV5(final String acceleratorClass, final Document document, final String md5) {
		if (acceleratorClass == null) {
			throw new IllegalArgumentException("accelerator class is null");
		}
	...
	public void bindHTML(final String id, final Adapter adapter) {
	 ...
	 */
	@SuppressWarnings("rawtypes")
	public void bindScript(final String id, final Adapter adapter) {
		if (adapter instanceof ListAdapter) {
						...
						for (final Method method : methods) {
							if (method.isAnnotationPresent(GetMethod.class)) {
								...
								if ("String".equals(returnType)) {
									...
									value = String.format(SIMPLE_FORM_INTEGER_VALUE, data1, getField); // , format, placeholder
								} else if ("Long".equals(returnType) || "long".equals(returnType)) {
									value = String.format(SIMPLE_FORM_LONG_VALUE, data1, getField); // , format, placeholder
								} else if ("Float".equals(returnType) || "float".equals(returnType)) {
									value = String.format(SIMPLE_FORM_FLOAT_VALUE, data1, getField); // , format, placeholder
								} else if ("Double".equals(returnType) || "double".equals(returnType)) {
									value = String.format(SIMPLE_FORM_DOUBLE_VALUE, data1, getField); // , format, placeholder
								} else if ("Date".equals(returnType)) {
			...
			if (newClazz != null) {
			    ...
			    if (oldMethod != null) {
					...
					executer.execute(new Runnable() {
						...
						public void run() {
							try {
								newClass.writeFile(getClass().getClassLoader().getResource("").getPath());
								newClass.defrost();
