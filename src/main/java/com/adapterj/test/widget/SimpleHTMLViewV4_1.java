 ...
 */
public class SimpleHTMLViewV4_1 extends AbstractView {

	private static final boolean DEBUG = Debugger.DEBUG ? false : false;
    private static final String TAG = SimpleHTMLViewV4_1.class.getName();

    private static final String LIST_VAR_LINE1 = "final String id = \"%s\";";
    private static final String LIST_VAR_LINE2 = "final com.adapterj.widget.SimpleListAdapter %s = getSimpleListAdapter(id);";
    private static final String LIST_VAR_LINE3 = "final java.util.List %s = %s.getAllItems();";
    private static final String LIST_VAR_LINE4 = "final java.util.List %s = %s.getAllLinkGroup();";
    private static final String LIST_VAR_LINE5 = "final String idOfSelectOptions%d%d = \"%s\";";
    private static final String LIST_VAR_LINE6 = "final com.adapterj.widget.SelectOptions options%d%d = %s.getSelectOptions(idOfSelectOptions%d%d);";
    private static final String LIST_FOR_LOOP1 = "<!--< for (int %s = 0; %s < %s.size(); %s ++) { >-->";
    private static final String LIST_FOR_LOOP2 = "<!--< } >-->";
    
    private static final String LIST_NULL_VALUE = "<!--<= %s.getPlaceholderForNull() >-->";
    private static final String LIST_STRING_VALUE = "<!--<= ((%s) %s.get(%s)).%s() >-->";
    private static final String LIST_BOOLEAN_VALUE = "<!--<= ((%s) %s.get(%s)).%s() >-->";
    private static final String LIST_INTEGER_VALUE = "<!--<= ((%s) %s.get(%s)).%s() >-->";
    private static final String LIST_LONG_VALUE = "<!--<= ((%s) %s.get(%s)).%s() >-->";
    private static final String LIST_DATE_VALUE = "<!--<= _formatter.format((java.util.Date) ((%s) %s.get(%s)).%s()) >-->";
    private static final String LIST_OBJECT_VALUE = "<!--<= ((%s) %s.get(%s)).%s() >-->";
    private static final String LIST_OPTION_VALUE = "<!--<= %s.getSelectOptions(\"%s\").selected(String.valueOf(((%s) %s.get(%s)).%s())) >-->";
    private static final String LIST_LINK_VALUE = "<!--<= ((com.adapterj.widget.Link) ((com.adapterj.widget.LinkGroup) %s.get(%s)).link(%d)).%s() >-->";
	...
	private final Map<String, String> _java = new HashMap<String, String>();
	 ...
	 * 
	 */
	public SimpleHTMLViewV4_1() {
		try {
			_acceleratorClass = SimpleHTMLViewAcceleratorV4_1.class.getName();
	 ...
	 */
	public SimpleHTMLViewV4_1(final String acceleratorClass) {
	 ...
	 */
	public SimpleHTMLViewV4_1(final Document document) {
		if (document == null) {
	 ...
	 * @param document The HTML template document
	 */
	public SimpleHTMLViewV4_1(final String acceleratorClass, final Document document) {
		if (acceleratorClass == null) {
			throw new IllegalArgumentException("accelerator class is null");
		}
		...
		if (newClazz != null) {
			...
			if (itemCount == 0) {
				...
				if (_java.get(id) == null) {
					s.delete(0, s.length());
					s.append(String.format(LIST_VAR_LINE1, id)).append('\n');
					s.append(String.format(LIST_VAR_LINE2, adapter1)).append('\n');
					s.append(String.format(LIST_VAR_LINE3, list11, adapter1)).append('\n');
					s.append(String.format(LIST_VAR_LINE4, list12, adapter1)).append('\n');
					
					final Set<String> idSetOfSelectOptions = adapter.idSetOfSelectOptions();
					int j = index1;
					int k = 1;
					for (String o : idSetOfSelectOptions) {
						s.append(String.format(LIST_VAR_LINE5, j, k, o)).append('\n');
						s.append(String.format(LIST_VAR_LINE6, j, k, adapter1, j, k)).append('\n');
						k ++;
					}
							...
							for (final Method method : methods) {
								if (method.isAnnotationPresent(GetMethod.class)) {
									...
									if (object == null) {
										...
										value = String.format(LIST_STRING_VALUE, type, list11, j1, getField);
									} else if ("Boolean".equals(returnType) || "boolean".equals(returnType)) {
										value = String.format(LIST_BOOLEAN_VALUE, type, list11, j1, getField);
									} else if ("Integer".equals(returnType) || "int".equals(returnType)) {
										value = String.format(LIST_INTEGER_VALUE, type, list11, j1, getField);
									} else if ("Long".equals(returnType) || "long".equals(returnType)) {
										value = String.format(LIST_LONG_VALUE, type, list11, j1, getField);
									} else if ("Date".equals(returnType)) {
										value = String.format(LIST_DATE_VALUE, type, list11, j1, getField);
									} else {
										value = String.format(LIST_OBJECT_VALUE, type, list11, j1, getField);
									}
									...
									if (options != null) {
										if (object == null) {
											...
											value = String.format(LIST_OPTION_VALUE, adapter1, returnId, type, list11, j1, getField);
										}
									}
...
//													Object object = null;
//													try {
//														object = mthd.invoke(link); // invoke getXXX method 
//													} catch (IllegalAccessException e) {
//														throw new BindException(e);
//													} catch (IllegalArgumentException e) {
//														throw new BindException(e);
...
//													}
													...
													final String value = String.format(LIST_LINK_VALUE, list12, j1, k, getField);
													java.put(elementId, value);
													if (DEBUG) {
											            StackTraceElement t = (new Throwable()).getStackTrace()[0];
				...
				if (!java.isEmpty()) {
					...
					element.before(String.format(LIST_FOR_LOOP1, j1, j1, list11, j1));
					element.before(html);
	...
	public String getAcceleratorClassName() {
		return _acceleratorClass + "_"; // 与之前版本不同的重要变化 by York/GuangYu DENG in 2019.08.10 10:59 
	}
	...
	public String toHTMLString() {
		...
		try {
			...
			if (newClazz == null) {
				...
				// extends SimpleHTMLViewAccelerator
				...
				final String javaCode = s.toString(); // Log.i(TAG, javaCode);
