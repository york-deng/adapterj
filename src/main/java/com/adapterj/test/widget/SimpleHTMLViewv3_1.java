 ...
 */
public class SimpleHTMLViewv3_1 extends AbstractView {

	private static final boolean DEBUG = Debugger.DEBUG ? false : false;
	
    private static final String TAG = SimpleHTMLViewv3_1.class.getName();
	 ...
	 */
	public SimpleHTMLViewv3_1(final String acceleratorClass) {
	 ...
	 */
	public SimpleHTMLViewv3_1(final String acceleratorClass, final Document document) {
		if (acceleratorClass == null) {
			throw new IllegalArgumentException("accelerator class is null");
		}
	...
	protected final void buildDOM(final String id, final ListAdapter adapter) {
		...
		if (itemCount == 0) {
			...
			if (javaCode == null) {
				...
				// final java.util.List list12 = adapter1.getAllLinkGroup();
				...
				// final String idOfSelectOptions11 = "type";
				// final com.adapterj.widget.SelectOptions options11 = adapter1.getSelectOptions(idOfSelectOptions11);
			...
			// 性能优化：可以保存成静态文本，然后通过 String.format(JAVA_IN_LIST_ADAPTER, list20, list20, ...) 的方式写入变量 
			s.delete(0, s.length());
			s.append("final").append(' ').append("String").append(' ').append("id");
			s.append(' ').append('=').append(' ');
			s.append('"').append(id).append('"').append(';').append('\n');
			
			s.append("com.adapterj.example.pojo.Source pojo = new com.adapterj.example.pojo.Source();").append('\n'); // for test
			s.append("String name = pojo.getName();").append('\n'); // for test
			
			s.append("final").append(' ').append("java.util.List").append(' ').append(list20); // DO NOT USE: java.util.List<java.util.List<String>>  
			s.append(' ').append('=').append(' ');
...
//													}
													...
													java.put(elementId, (String) value);
			...
			if (_original instanceof SimpleHTMLViewAccelerator) {
				//((SimpleHTMLViewAccelerator) _original).putList2(id, list2);
				//Log.i(TAG, String.format("id is %s, list2 is %s", id, ((SimpleHTMLViewAccelerator) _original).getList2(id)));
			...
			if (newClazz != null) {
				...
				if (!java.isEmpty()) {
				 ...
				 // element.before("<!--< for (int j1 = 0; j1 < 0; j1 ++) { >-->");
					element.before(html);
							...
							if (beginJava >= 0 && endJava >= 0) {
								while (beginJava >= 0 && endJava >= 0) {
									...
									try {
										...
										if (beginJava < 0 || endJava < 0) {
//											Log.i(TAG, "fromIndex is " + fromIndex);
											if (fromIndex < line.length()) {
												final String right = line.substring(fromIndex);
//												Log.i(TAG, "Right End: " + right);
												if (!right.isEmpty()) {
													s.append("s.append").append('(');
...
//									Log.i(TAG, "Last Line: " + line);
					...
					//final String javaCode = "public String toHTMLString() { final String id = \"_list.item\"; java.util.List list21 = getList2(id); com.adapterj.example.pojo.Source pojo = new com.adapterj.example.pojo.Source(); return \"OK!!!\"; }"; // 仅用于测试 
...
//			out2.writeObject(_original);
//			byte[] objectBytes = out1.toByteArray();
//			out2.close();
//			out1.close();
//			
//			ByteArrayInputStream in1 = new ByteArrayInputStream(objectBytes);
//			ObjectInputStream in2 = new AdapterJObjectInputStream(in1, classLoader);
//			final Object boost = in2.readObject();
//			in2.close();
//			in1.close();
